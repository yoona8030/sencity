...
// 🔐 DEV 전용 — 배포 전 삭제!
const DEV_USERNAME = 'yuna'; // ← 너의 관리자 아이디
const DEV_PASSWORD = 'yuna'; // ← 관리자 비번
...
export default function Inquiry() {
    ...
    (async () => {
      ...
      try {
        ...
        if (res.ok) {
          const m = await res.json();
          setMe({ username: m?.username, is_staff: !!m?.is_staff });
          setAuthProblem('none'); // ✅ 문제 없음
        } else if (res.status === 401) {
          setMe(null);
          setAuthProblem('unauth'); // ✅ 인증 안됨(토큰 문제)
        } else if (res.status === 403) {
          setMe(null);
          setAuthProblem('forbidden'); // ✅ 권한 부족
        } else {
          setMe(null);
          setAuthProblem('unauth'); // 기타는 일단 인증 문제로 취급
        }
      } catch {
        setMe(null);
    ...
    try {
      ...
      if (!listRes.ok) {
        if (listRes.status === 404) {
          throw new Error('서버 라우팅(문의 API)이 설정되지 않았습니다.'); // 필요시 DEV 전용
        }
      ...
      // 2) staff인데 0건이면(토큰은 정상) 안전하게 한 번 더 시도 (백엔드 커스텀 필터 대비)
  ...
  }, [tab, me?.is_staff]); // staff 여부 바뀌면 재로딩
      ...
      {__DEV__ && authProblem !== 'none' && (
        <View style={st.devBanner}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            <Text style={{ fontSize: 12, fontWeight: '700', color: '#7A4B00' }}>
              개발 로그인
            </Text>
          </TouchableOpacity>
        </View>
